...
const AdminBlog = () => {
  ...
  const loadBlogPosts = async () => {
    try {
      setLoading(true);
      const posts = await blogService.readAll<any>();
      const normalized = posts.map((p: any) => ({
        ...p,
        publishDate: p.publishDate?.toDate ? p.publishDate.toDate() : (p.publishDate || new Date()),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        ...formData,
        publishDate: formData.publishDate instanceof Date ? Timestamp.fromDate(formData.publishDate) : formData.publishDate,
      };
      
      if (isEditModalOpen && selectedPost?.id) {
        await blogService.update(selectedPost.id, payload);
        setBlogPosts(prev => prev.map(post => 
          post.id === selectedPost.id ? { ...formData, id: selectedPost.id } : post
        ));
      } else {
        const newId = await blogService.create(payload);
  ...
  const togglePublishStatus = async (id: string) => {
    try {
      ...
      await blogService.update(id, { isPublished: nextPublished } as any);
      setBlogPosts(prev => prev.map(post => 
        post.id === id ? { ...post, isPublished: nextPublished } : post
      ));
    } catch (error) {
